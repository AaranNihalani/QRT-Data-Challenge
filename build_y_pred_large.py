...
# -----------------------------
# DeepSurv implementation (CPU-safe)
# -----------------------------
class DeepSurvModel(nn.Module):
    ...
    def forward(self, x):
        z = self.backbone(x)
        out = self.head(z)
        return out.squeeze(-1)  # log-risk


class DeepSurvLoss(nn.Module):
    """Negative log partial likelihood for Cox model.
    y_time: durations, y_event: 1 if event, 0 if censored.
    """
    def forward(self, risk: torch.Tensor, y_time: torch.Tensor, y_event: torch.Tensor):
        # sort by time descending
        order = torch.argsort(y_time, descending=True)
        risk_sorted = risk[order]
        event_sorted = y_event[order]
        # log cumulative hazard via logsumexp
        log_cum_hazard = torch.logcumsumexp(risk_sorted, dim=0)
        # contribution only for events
        losses = (risk_sorted - log_cum_hazard) * event_sorted
        return -losses.sum() / (event_sorted.sum() + 1e-8)
...
def train_deepsurv(X_tr: np.ndarray, y_tr_df: pd.DataFrame, X_val: np.ndarray, y_val_df: pd.DataFrame,
    ...
    model = DeepSurvModel(input_dim=X_tr.shape[1], hidden_dims=hidden_dims, dropout=dropout).to(device)
    criterion = DeepSurvLoss()
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)

    def _prep(X: np.ndarray, y_df: pd.DataFrame):
        durations = _get_y_indexed(y_df, pd.Index([str(i) for i in y_df.index]))["OS_YEARS"].values.astype(np.float32)
        events = _get_y_indexed(y_df, pd.Index([str(i) for i in y_df.index]))["OS_STATUS"].values.astype(np.float32)
        return torch.tensor(X, dtype=torch.float32), torch.tensor(durations), torch.tensor(events)

    # Align y to X indices we pass in; here we assume X_tr rows align to y_tr_df.index
    X_tr_t = torch.tensor(X_tr, dtype=torch.float32).to(device)
    X_val_t = torch.tensor(X_val, dtype=torch.float32).to(device)
    y_tr_dur = torch.tensor(y_tr_df["OS_YEARS"].values.astype(np.float32)).to(device)
    y_tr_evt = torch.tensor(y_tr_df["OS_STATUS"].values.astype(np.float32)).to(device)
    y_val_dur = torch.tensor(y_val_df["OS_YEARS"].values.astype(np.float32)).to(device)
    y_val_evt = torch.tensor(y_val_df["OS_STATUS"].values.astype(np.float32)).to(device)
...
def predict_deepsurv_risk(model: nn.Module, X: np.ndarray) -> np.ndarray:
    if not TORCH_AVAILABLE:
    ...
    with torch.no_grad():
        ...
        risk = model(X_t).detach().cpu().numpy()
    return risk
